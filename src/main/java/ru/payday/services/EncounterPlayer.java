...
public class EncounterPlayer {
    ...
    public Cop createCop() {
        ...
        return new Cop(copName.get(random.nextInt(copName.size())), random.nextInt(ageDelta) + minAge,
                copNationality.get(random.nextInt(copNationality.size())),
                copGender.get(random.nextInt(copGender.size())),
                rankProbabilities.get(random.nextInt(rankProbabilities.size())));
    }
